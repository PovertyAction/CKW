...
Purpose: Analysis on Quantity part
***************/
clear all
capture log close
...
/*
preserve
collapse propComplete totalcomplete ckw_ACTED ckw_BEDNET ckw_FORESTRY ckw_LOAN ckw_NRC ckw_condom ///
		ACTED BEDNET FORESTRY LOAN NRC condom_received condom_received_miss educationlevel gender_enum ///
		age_enum ckw_local ckw_sense  ///
		data_local data_sense know_ACTED know_BEDNET know_FORESTRY know_LOAN know_NRC know_condom ///
		knowresp samevillage_ACTED samevillage_BEDNET samevillage_FORESTRY samevillage_LOAN samevillage_NRC samevillage_condom ///
		, by(ckw enumerator_full)		
...
**redo main table but with for depvar#2, unanswered questions are deemed "inaccurate"
replace recorded_accurate = 0 if mi(recorded_accurate) & !mi(recorded_answer)
...
foreach var of local outcomes{
	...
	if "`var'" == "recorded_answer" {
		...
		qui estadd loc unit = "Question on Attempted Survey for Which We Have ^!Truth^! from Administrative Data"
	}
	else if "`var'" == "recorded_accurate" {
		qui estadd loc sample = "Completed Questions"
		qui estadd loc unit = "Completed Question on Attempted Survey for Which We Have ^!Truth^! from Administrative Data"
	}
...
esttab A*1 using "`dest'/reg_accMISS0_ols.csv", b(3) se(3) ar2(2) title(Table 2: Main Results) ///
