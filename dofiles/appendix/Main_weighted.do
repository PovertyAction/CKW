...
Purpose: Generate Appendix tables for weighted regression of main table: weight = 
	divide the outcome vars by (1) # of attempted questions and (2) # of completed questions
	This is to give the equal weight to each respondent
***************/
clear all
capture log close
...
egen nm = rownonmiss(recorded_answer)
bys uniqueid: egen numnm = total(nm)
egen nm_acc = rownonmiss(recorded_accurate)
bys uniqueid: egen numnm_acc = total(nm_acc)

bys uniqueid: replace recorded_answer = recorded_answer / numnm
bys uniqueid: replace recorded_accurate = recorded_accurate / numnm_acc
...
esttab A*1 using "`dest'/reg_mainWeight_ols.csv", b(3) se(3) ar2(2) title(Table 2: Main Results) ///
